...
# NOVA ROTA PARA O MONITOR DE PINTURA
...
def get_production_data():
    ...
    if fase in [25, 30, 35, 998, 999] and not df_processed.empty:
        ...
        df_summary = df_details.groupby('op_group').agg(
...
def get_devolucoes_data():
    ...
    query = f"""
        ...
        devolucoes AS (
            SELECT
                m.priordem, m.priproduto,
                SUM(m.priquanti) as quanti_devolvida,
                MAX(m.pridata) as ultima_data_devolucao
            FROM {fq('toqmovi')} m
            WHERE m.pritransac = '4'
              AND m.priobserv ILIKE '%%*d:%%'
              AND m.pridata >= '2025-01-01'
            GROUP BY m.priordem, m.priproduto
        ),
        debitos_devolucao AS (
            SELECT
                m.priordem, m.priproduto,
                SUM(m.priquanti) as quanti_debitada
            FROM {fq('toqmovi')} m
            WHERE m.pritransac = '14'
              AND m.priobserv ILIKE '%%*d:%%'
              AND m.pridata >= '2025-01-01'
            GROUP BY m.priordem, m.priproduto
        ),
        saldo_devolucoes AS (
            SELECT
                d.priordem, d.priproduto, d.ultima_data_devolucao,
                GREATEST(d.quanti_devolvida - COALESCE(deb.quanti_debitada, 0), 0) as saldo_pendente
            FROM devolucoes d
            LEFT JOIN debitos_devolucao deb
                ON TRIM(CAST(d.priordem AS TEXT)) = TRIM(CAST(deb.priordem AS TEXT))
                AND TRIM(d.priproduto) = TRIM(deb.priproduto)
        )
        SELECT
            l.lotdes as lote_descricao,
            p.pronome as descricao,
            sd.ultima_data_devolucao as data,
            sd.saldo_pendente as quantidade
        FROM saldo_devolucoes sd
        JOIN {fq('produto')} p ON TRIM(p.produto) = TRIM(sd.priproduto)
        JOIN {fq('ordem')} o ON TRIM(CAST(o.ordem AS TEXT)) = TRIM(CAST(sd.priordem AS TEXT))
        JOIN {fq(lot_table)} l ON o.lotcod = l.lotcod
        WHERE sd.saldo_pendente > 0 AND {op_filter}
        ORDER BY sd.ultima_data_devolucao DESC;
    """
