...
def _parse_phase_dates(text: str, phase_name: str):
    ...
    try:
        ...
        # Handle new TAPECARIA(136) format
        if phase_name == 'TAPECARIA':
            # This pattern now looks for a start and end date
            ...
            # Fallback for single date format if it still exists in some records.
        ...
        # Original pattern for other phases
        pattern_default = rf"\b{re.escape(phase_name)}\b:\s*(\d{{2}}/\d{{2}}/\d{{2,4}})\s*-\s*(\d{{2}}/\d{2}/\d{2,4})"
        m_default = re.search(pattern_default, str(text), flags=re.IGNORECASE)
        if m_default:
            return to_iso(m_default.group(1)), to_iso(m_default.group(2))
...
def process_data(df: pd.DataFrame, fase: int) -> pd.DataFrame:
    ...
    # Lógica específica para o monitor de Tapeçaria (fase 136)
    if fase == 136:
        df_atrasado = df[df['status'] == 'atrasado']
        
        # OPs que não estão atrasadas (podem ser 'em_dia' ou 'futuro')
        df_nao_atrasado = df[df['status'] != 'atrasado'].copy()
        
        df_em_dia = pd.DataFrame() # Começa com um dataframe vazio para os itens em dia

        if not df_nao_atrasado.empty:
            # Seleciona apenas as ordens que já deveriam ter iniciado e ainda não terminaram
            ...
            if not df_ativas_ou_futuras_proximas.empty:
                ...
                # Se nenhuma OP está ativa, pega a próxima a iniciar
        ...
        # Combina as OPs atrasadas com as OPs em dia (agora corretamente filtradas)
    ...
    if fase == 136:
        ...
        return f"""
            ...
            ordens_com_fase AS (
                ...
                WHERE pr.prccodig = '136' AND o.orddtence = DATE '0001-01-01'
            ),
...
def get_production_data():
    ...
    # Para os monitores que agrupam OPs (Maciço, Chapa, Pintura, Saída Pintura, Saída Montagem, Tapeçaria)
    if fase in [25, 30, 35, 136, 998, 999] and not df_processed.empty:
        ...
        for df_to_format in [df_summary, df_details]:
            if not df_to_format.empty:
                sort_cols = ['corte_dtini'] if 'corte_dtini' in df_to_format.columns else ['orddtprev']
    ...
    if not df_processed.empty:
        sort_cols = ['corte_dtini'] if 'corte_dtini' in df_processed.columns else ['orddtprev']
...
def get_completed_data():
    ...
    ord_col, qtd_col = _pasfase_columns()
    if fase == 136 and table_exists('planilha'):
        qtd_fase_source = f"""
            SELECT CAST(plaordem AS TEXT) AS ordem, SUM(COALESCE(CAST(plaquant AS NUMERIC), 0)) AS qtd_produzida
            FROM {fq('planilha')} WHERE plaopera = '136' GROUP BY plaordem
        """
    elif fase == 35 and table_exists('planilha'):
        qtd_fase_source = f"""
            SELECT CAST(plaordem AS TEXT) AS ordem, SUM(COALESCE(CAST(plaquant AS NUMERIC), 0)) AS qtd_produzida
            FROM {fq('planilha')} WHERE plafase = %(fase)s GROUP BY plaordem
        """
    ...
    query = f"""
        ...
        ordens_filtradas AS (
            SELECT 
                ...
                l.lotdes as lote_descricao
            FROM {fq('ordem')} o
            JOIN {fq(lot_table)} l ON o.lotcod = o.lotcod
            WHERE EXISTS (
                SELECT 1 FROM {fq('processo')} pr 
                WHERE pr.produto = o.ordproduto AND (pr.fase = %(fase)s OR pr.prccodig = CAST(%(fase)s AS TEXT))
            )
            {lote_filter_clause}
        )
        SELECT 
...
def export_data():
    ...
    phase_map = {
        5: 'Corte', 10: 'Prensa', 15: 'Usinagem', 25: 'Maciço', 30: 'Chapa', 35: 'Pintura',
        136: 'Tapecaria',
